...
| `GrassBlades` | Cỏ 3D nhú lên (tier B) — InstancedMesh lá geometry, accent-only+cap; cặp `GrassGround` tier A. 2 mặt 2 màu + bụi + vệt-tiếp-đất theo nắng + **MIX nhiều loài (`species[]` — mỗi loài 1 mesh hình/màu/weight riêng, HISM-per-foliage)** | vegetation, instanced, grass, cỏ, tier-b, site-kit, multi-species, foliage-mix | ✅ in-use |
